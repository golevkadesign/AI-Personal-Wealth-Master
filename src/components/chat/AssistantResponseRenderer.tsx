...
export const AssistantResponseRenderer: React.FC<AssistantResponseRendererProps> = ({
  ...
  // 如果处于流式模式或编译失败或被判定为 fallback，则直接采用传统简洁 Markdown 气泡形式
  if (isStreaming || !viewModel || viewModel.meta.isFallback) {
    return (
  ...
  // 计算多维卡片格网
  const hasRisk = viewModel.tone === 'risk' || viewModel.tone === 'mixed';
  const hasOpportunity = viewModel.tone === 'opportunity' || viewModel.tone === 'mixed';
  const hasAction = viewModel.tone === 'action' || viewModel.tone === 'mixed';
  ...
  return (
    <div className="w-full text-left font-sans flex flex-col space-y-4 select-text">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {viewModel.meta.estimatedReadMinutes && (
            <span className="bg-[#12151A] px-2 py-0.5 rounded border border-[#1C2026] flex items-center gap-1 text-[#8C8370]">
              ...
              {viewModel.meta.estimatedReadMinutes} min read
            </span>
          )}
      ...
      {/* Main Struct blocks mapper */}
      <div className="space-y-4">
        {viewModel.blocks.map((block, idx) => {
          switch (block.type) {
            case 'summary':
              return (
                ...
                <div key={idx} className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                    {block.cards.map((card) => {
                      ...
                      const IconComp = toneConfig.icon;
            ...
            case 'table':
              return (
                <div key={idx} className="space-y-1.5">
                  ...
                  <div className="overflow-x-auto rounded-xl border border-[#1C2026] bg-[#0E1114]">
      ...
      <div className="border-t border-[#1C2026] pt-3 mt-1 text-left">
        <div className="flex items-center justify-between">
          <button
            ...
            className="inline-flex items-center gap-1.5 text-[11px] font-mono text-[#8C8370] hover:text-[#C9B284] transition-colors"
          >
            <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isRawExpanded ? 'rotate-180' : ''}`} />
            <span>{isRawExpanded ? '收起完整原文' : '查看完整原文'}</span>
          </button>

          <button
            onClick={handleCopyRaw}
            className="inline-flex items-center gap-1 text-[11px] font-mono text-[#8C8370] hover:text-[#C9B284] transition-colors"
            title="复制 AI 分析原文"
          >
            {isCopied ? (
              <>
