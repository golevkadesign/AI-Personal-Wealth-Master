...
export function ChartWidget({ title, type, dataLength, insight, option, delay = 0, chartHeight = '250px', children, status, onReload, showReload, reloadLabel, isReloading, badge, onChartClick, className, size }: ChartWidgetProps) {
  ...
  return (
    <motion.div
      ...
      className={`aw-chart-shell aw-module-data p-4 sm:p-5 flex flex-col relative overflow-hidden group ${sizeClassMap[size || 'md']} ${className || 'h-full'}`}
    >
      ...
      {currentStatus === 'empty' && (
         <div className="aw-chart-empty-state flex-1 flex flex-col items-center justify-center aw-text-tertiary">
           <div className="aw-empty-chart-bars mb-5" aria-hidden="true">
             {Array.from({ length: 18 }).map((_, index) => (
	               <span key={index} />
             ))}
           </div>
           <MaterialIcon name="donut_large" size={32} className="mb-3 opacity-45" />
