...
export function useTerminalSync() {
      ...
      if (u) {
        try {
          const localDataStr = localStorage.getItem(
            `ai_terminal_data_${u.uid}`,
          );
          const localSnapshotsStr = localStorage.getItem(
            `ai_terminal_snapshots_${u.uid}`,
          );

          if (localSnapshotsStr) {
            try {
              useWealthStore.setState({
                agentMemorySnapshots: JSON.parse(localSnapshotsStr),
              });
            } catch (e) {}
          }
            ...
            unsubscribeSnapshot = onSnapshot(
              ...
              (snapshot) => {
                if (snapshot.exists()) {
                  ...
                  if (
                    fsData.appData &&
                    Object.keys(fsData.appData).length > 0
                  ) {
                  ...
                  if (fsData.userProfile) {
                    localState = {
                      ...localState,
                      userProfile: fsData.userProfile,
                    };
                  } else if (!fsData.appData && !fsData.chatHistory) {
                    localState = { ...localState, userProfile: fsData };
                  }

                  const cleaned = sanitizeTerminalState(
                    localState,
                  ) as TerminalState;
                  setData(normalizeDashboardSchema(cleaned) as TerminalState);
                } else {
                  console.log(
                    "Terminal: Remote document deleted, resetting local state.",
                  );
                  setData(EMPTY_STATE);
                }
                setLoadingAuth(false);
  ...
  const fetchLongbridgeAccountPortfolios = useWealthStore(
    (state) => state.fetchLongbridgeAccountPortfolios,
  );
